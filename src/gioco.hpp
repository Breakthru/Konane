...
template <class tipo_statodelgioco, class tipo_mossa> class gioco {
  // la classe virtuale gioco si basa su due tipi di dati virtuali,
  // tipo_statodelgioco e tipo_mossa
...
public:
  // funzioni virtuali, devono essere ridefinite a seconda dell'oggetto che si
  // vuole costruire

  virtual int valuta(tipo_statodelgioco stato) {
    std::cout << "errore: valuta non e' stato definito\n";
  }
  // esegue la funzione di valutazione di uno stato

  virtual lista<tipo_mossa> listamosse(tipo_statodelgioco stato,
                                       tipo_nodo tipo) {
    std::cout << "errore: listamosse non e' stato definito\n";
  }
  // restituisce la lista delle mosse possibili su uno stato per un giocatore
  // (min o max)

  virtual tipo_statodelgioco eseguimossa(tipo_statodelgioco stato,
                                         tipo_mossa mossa) {
    std::cout << "errore: eseguimossa non e' stato definito\n";
  }
  // esegue la mossa specificata sullo stato del gioco per restituire un nuovo
  // stato del gioco
  ...
  int alfabeta(tipo_statodelgioco stato, int livello, int alpha, int beta,
               tipo_nodo tipo);
...
int gioco<tipo_statodelgioco, tipo_mossa>::alfabeta(tipo_statodelgioco stato,
                                                    int livello, int alpha,
                                                    int beta, tipo_nodo tipo) {
  ...
  // std::cout << "ci sono " << possibili_mosse.numero() << " mosse da qui per "
  // << tipo << endl;
  ...
  if (tipo == min_) {
    for (i = 0; i < possibili_mosse.numero(); i++) {
      figlio = eseguimossa(
          stato, possibili_mosse.estrai()); // questo sviluppa il figlio
...
template <class tipo_statodelgioco, class tipo_mossa>
tipo_mossa
gioco<tipo_statodelgioco, tipo_mossa>::decidiMossa(tipo_statodelgioco gioco) {
  ...
  if (taglio_mossa_vincente) {
    testo << "\ntaglio su mossa vincente \n";
  }
  testo << "\n\nEsplorati " << n_esplorati << " nodi, " << n_valutati
