...
void interfaccia_grafica::popup(char *titolo, char *testo) {
  ...
  gtk_signal_connect(GTK_OBJECT(bottone), "clicked", G_CALLBACK(distruggiPopup),
                     finestra);
...
void imp_Stato_Pedina(int q, int i, int j, interfaccia_grafica *pgioco,
                      gchar *file) {
  xdelay *aux;
  aux = new (xdelay);
  aux->i = i;
...
void comesigioca(GtkWidget *wid, gpointer data) {
  ...
  gtk_text_buffer_insert_at_cursor(
      textbuffer,
      "Il giocatore ha le pedine nere, il computer quelle bianche. Come mossa "
      "iniziale scegli una pedina da rimuovere.\n",
      -1);
  gtk_text_buffer_insert_at_cursor(
      textbuffer,
      "Il computer eliminera' una sua pedina adiacente a quella rimossa. "
      "Dopodiche' il giocatore inizia a muovere. Le uniche mosse consentite "
      "sono quelle di saltare ",
      -1);
  gtk_text_buffer_insert_at_cursor(
      textbuffer,
      "una pedina avversaria per mangiarla. Si puo' mangiare una pedina in "
      "orizzontale o in verticale. E' possibile anche mangiare piu' pedine, ma "
      "senza cambiare direzione.\n",
      -1);
  gtk_text_buffer_insert_at_cursor(
      textbuffer,
      "Per effettuare una mossa clicca su una tua pedina, poi clicca sulla "
      "casella di destinazione. Se vuoi mangiare piu' pedine, clicca "
      "direttamente sulla casella finale.\n",
      -1);
  gtk_text_buffer_insert_at_cursor(
      textbuffer,
      "Il gioco prosegue fin quando uno dei giocatori rimane senza mosse "
      "disponibili, e perde. Non accade mai un pareggio. \n",
      -1);
  gtk_text_buffer_insert_at_cursor(
      textbuffer,
      "Sulla destra vengono mostrate le mosse che hai a disposizione, e i "
      "calcoli che effettua il computer prima di muovere.\n",
      -1);
}
...
interfaccia_grafica::interfaccia_grafica(konane *pgioco) {
  ...
  g_signal_connect(G_OBJECT(selettore), "value-changed", G_CALLBACK(impLivello),
                   pgioco);
  ...
  g_signal_connect(G_OBJECT(newgame), "clicked", G_CALLBACK(nuova_partita),
                   pgioco);
  ...
  for (j = 1; j <= NUMERO_CELLE_VERTICALI; j++) {
    ...
    for (i = 1; i <= NUMERO_CELLE_ORIZZONTALI; i++) {
      ...
      g_signal_connect(G_OBJECT(bottone), "clicked", G_CALLBACK(cliccato),
                       &scacchiera[i][j]);
  ...
  g_signal_connect(G_OBJECT(finestra), "destroy", G_CALLBACK(gtk_main_quit),
                   NULL);
...
void interfaccia_grafica::cliccato(GtkWidget *wid, gpointer data) {
  ...
  if (DEBUG)
    stampa_scacchiera(gioco->stato);
  // cout << "bottone cliccato " << x << " " << y << endl;

  if ((!gioco->stato.casella[x][y].occupata) &&
      (!gioco->stato.pedina_selezionata))
    return;
  if ((gioco->stato.casella[x][y].occupata) &&
      (gioco->stato.casella[x][y].appartiene != giocatore))
    return;
  // casi in cui non devo fare nulla.

  if (gioco->numeromosse ==
      0) { // elimina la pedina scelta dal giocatore nella prima mossa
  ...
  if (gioco->stato.pedina_selezionata) { // il giocatore ha indicato una mossa
    i = gioco->stato.sele.x;
    j = gioco->stato.sele.y;
    // cout << "la pedina sele attualmente e' " << i << ","<<j << endl;
    if ((x == i) &&
        (y == j)) { // cliccare la pedina selezionata significa DEselezionarla.
      // cout << "il giocatore ha DEselezionato la pedina " << i <<"," << j <<
      // endl;
    ...
    mossa.i1 = x;
    mossa.j1 = y;
    if (mossa_valida(gioco->stato, mossa,
                     giocatore)) { // il giocatore ha indicato una mossa valida
    ...
    // cout << "il giocatore ha selezionato la pedina " << x << " , " << y <<
    // endl;
