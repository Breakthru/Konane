...
{
cout << "mossa da (" << mossa.i << "," <<mossa.j <<") a (" <<mossa.i1<<","<<mossa.j1<<")" << endl;
}
...
{ // decidere la pedina che il computer deve inizialmente togliere
...
{
cout << "il giocatore fa la  "; printmossa(mossa);
...
ostringstream testo;
testo << "il computer ha mosso (" << mossaComputer.i << "," << mossaComputer.j << ") ---> (" << mossaComputer.i1 << "," << mossaComputer.j1 << ")";
...
nPc = temp.numero();
n_valutati++;
if(nPc==0) return(-INFINITO);
if(nPlayer==0) return(INFINITO);
return (nPc - nPlayer);
}
...
temp=g.estrai();
testo << "(" << temp.i << "," << temp.j << ") ---> (" << temp.i1 << "," << temp.j1 << ")\n";
}
testo << endl;
...
bool appartiene(cella &a,player chimuove)
{
if (a.occupata==false) return (false);
else if (a.appartiene==chimuove) return(true);
else return(false);
}

player inverso(player a)
{if (a==computer) return (giocatore);
else if (a==giocatore) return (computer);
else cout << "errore. giocatore non definito\n" << endl;
}

void trovaCandidatoMossaMultipla(int i,int j,int i1,int j1,int &i2, int &j2)
{
i2=i1;
j2=j1;
if (j==j1) { //mosse orizzontali
if (i>i1) { j2=j;i2=i1+2;}
if (i<i1) {j2=j;i2=i1-2;}
} //fine mosse orizzontali
else if (i==i1) { //mosse verticali
if (j>j1) {i2=i;j2=j1+2;}
if (j<j1) {i2=i;j2=j1-2;}
...
bool saltaunacella(tipo_statodelgioco &stato,const tipo_mossa &mossa,player occupatada)
{
if (mossa.i==mossa.i1) //salto in verticale
{if( (mossa.j1 == mossa.j +2 ) && appartiene(stato.casella[mossa.i][mossa.j+1],occupatada) ) return(true);
if( (mossa.j1 == mossa.j -2 ) && appartiene(stato.casella[mossa.i][mossa.j-1],occupatada) ) return(true);
}//fine salto verticale

if (mossa.j==mossa.j1) //salto orizzontale
{if( (mossa.i1 == mossa.i +2 ) && appartiene(stato.casella[mossa.i+1][mossa.j],occupatada) ) return(true);
if( (mossa.i1 == mossa.i -2 ) && appartiene(stato.casella[mossa.i-1][mossa.j],occupatada) ) return(true);
}//fine salto verticale
...
bool mossa_valida(tipo_statodelgioco &stato,const tipo_mossa &mossa,player chimuove)
{//controlla che la mossa sia valida per chimuove sullo stato.
int i=mossa.i;
int i1=mossa.i1;
int j=mossa.j;
int j1=mossa.j1;
...
if( saltaunacella(stato,mossa,avversario) )return(true);

if (i==i1) {if ( (abs(j-j1) % 2) !=0 ) return(false);}
if (j==j1) {if ( (abs(i-i1) % 2) !=0 ) return(false);}

trovaCandidatoMossaMultipla(i,j,i1,j1,i2,j2);
if( (i==i2)&&(j==j2) ) return(false);
tipo_mossa semplice,resto;
semplice.i=i; semplice.j=j; semplice.i1=i2; semplice.j1=j2;
resto.i=i2; resto.j=j2; resto.i1=i1;resto.j1=j1;
return(mossa_valida(stato,semplice,chimuove) && mossa_valida(stato,resto,chimuove));

}

void mossePossibili(tipo_statodelgioco &stato,int x,int y,lista<tipo_mossa> &lista,player chimuove)
{
int i=x,j=y;
tipo_mossa temp;
for (i=1;i<=numero_celle_orizzontali;i++)
...
temp.i1=x;
temp.j1=y;
if (mossa_valida(stato,temp,chimuove)) {lista.inserisci(temp);
//cout << "trovato una mossa possib " << temp.i<<temp.j<< " " <<temp.i1<<temp.j1<<endl;
}
}}
i=x;j=y;
for (j=1;j<=numero_celle_verticali;j++)
...
{
if (tipo==min_) return(giocatore);
else if (tipo==max_) return(computer);
else cout << "errore, tipo non vale ne' min ne' max\n" << endl;
}
...
{// trova le coordinate in base alla direzione
mangia_x=mossa.i;
mangia_y=mossa.j;
presa_x=mossa.i;
presa_y=mossa.j;
...
mangia_x=mossa.i-1; mangia_y=mossa.j; // mangiata la pedina ad ovest
presa_x=mossa.i-2;presa_y=mossa.j; // prendo la casella a due ovest
}// fine mossa verso ovest 
if( mossa.i<mossa.i1 ) { // mossa verso est 
mangia_x=mossa.i+1; mangia_y=mossa.j; // mangiata la pedina ad est 
presa_x=mossa.i+2;presa_y=mossa.j; // prendo la casella a due est
...
mangia_x=mossa.i; mangia_y=mossa.j-1; // mangiata la pedina a nord 
presa_x=mossa.i;presa_y=mossa.j-2; // prendo la casella a due nord
}// fine mossa verso nord 
if( mossa.j<mossa.j1 ) { //mossa verso sud 
 mangia_x=mossa.i; mangia_y=mossa.j+1; // mangiata la pedina a sud 
presa_x=mossa.i;presa_y=mossa.j+2; // prendo la casella a due sud
...
void assegnapedina(cella &pedina,player a)
{//assegna la pedina al giocatore a
if (a==nessuno) pedina.occupata=false;
else {pedina.occupata=true; pedina.appartiene=a;}

}

tipo_statodelgioco konane::eseguimossa(tipo_statodelgioco stato,tipo_mossa mossa)
{
...
//cout << "da " << mossa.i<<"," << mossa.j << " mangio " << mangia_x<< ","<<mangia_y<< " e prendo "<<presa_x<<","<<presa_y<<endl;
if (!stato.casella[mossa.i][mossa.j].occupata) {cout << "errore!"; exit(1);}
assegnapedina(ret.casella[presa_x][presa_y],ret.casella[mossa.i][mossa.j].appartiene);
assegnapedina(ret.casella[mossa.i][mossa.j],nessuno);
assegnapedina(ret.casella[mangia_x][mangia_y],nessuno);

if( (mossa.i1 != presa_x) || (mossa.j1 != presa_y) ) { //esegui una mossa composta
tipo_mossa temp;
temp.i = presa_x; temp.j=presa_y;
...
if(DEBUG) stampa_scacchiera(stato);
